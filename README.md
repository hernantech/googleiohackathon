...
**SME execution note:** SMEs run as **per-SME Antigravity managed agents** (the default), via a **tool-capable hybrid**. Custom function tools work on a model-based `interactions.create` but the Antigravity base agent rejects them (verified live: `400 "Tool 'function' is not allowed"`), so each SME (a) **GATHERS** cited grounding with a bounded model-based tool-loop that calls the read-only knowledge tools plus `run_analysis` (real Python math) — each emitted function-call is executed against the per-session knowledge adapter, fed back over the continuation, and **streamed** to the chat bus as it happens — then (b) **REASONS** in its **own warm Antigravity sandbox** (a per-SME environment reused across turns) on the retrieved, cited briefing to produce the final answer. Each loads its persona/skill from `smes/<id>/` (AGENTS.md + SKILL.md). Documented-limit citations are attached by the orchestrator — the model never invents a setpoint. The per-SME envs are **prewarmed at startup** and **kept warm** (pinged ~240 s) so the ~70 s cold-start is off the critical path. The guild fans out **concurrently**. Set `FORGE_SME_USE_SANDBOX=0` to fall back to the fast pure `gemini-3.5-flash` tool-calling loop (also the automatic fallback on any sandbox error). The `run_analysis` compute tool is still backed by a single shared kept-warm sandbox. See [ROADMAP.md](ROADMAP.md).
